...
Build ligolo-ng proxy and agent binaries with the embedded web UI baked in.

The build follows upstream's own method: it clones [ligolo-ng](https://github.com/nicocha30/ligolo-ng) at a **pinned release tag together with its `ligolo-ng-web` submodule**, then runs `go generate ./web` — the author's directives that build the frontend into `web/dist`, which is embedded via `//go:embed`. This keeps the proxy and web UI in lock-step (the versions the author tested together) and makes builds reproducible, instead of hand-wiring the two repos at their moving `HEAD`.

Static binaries are compiled for Linux/Windows (amd64/arm64/386).
...
The pinned upstream version is set in the `Makefile` (`LIGOLO_VERSION`). To build a different release without editing files:
